...
class AnimatedGridView extends StatefulWidget {
  ...
  ///[crossAxisSpacing] space the cross  items
  ...
  ///[mainAxisSpacing] space the cross  items
  final double mainAxisSpacing;

  ///[childAspectRatio] space the cross  items
  final double childAspectRatio;

  ///[crossAxisCount] the gird view span count
  final int crossAxisCount;

  ///[extendedSpaceBetween] space the items when selected
  final double extendedSpaceBetween;
...
class _AnimatedGridViewState extends State<AnimatedGridView> {
  ...
  void initState() {
    super.initState();
    spaceBetween = widget.scrollDirection == Axis.horizontal
        ? widget.crossAxisSpacing
        : widget.mainAxisSpacing;

    // we will do the animations by steps to control the end scroll
    final step = widget.duration / (widget.extendedSpaceBetween - spaceBetween);

    _duration = Duration(milliseconds: step.toInt());
  }
  ...
  _onUpdateScroll(ScrollMetrics metrics) {
    // avoid fast user touches cliches
    if (spaceBetween == widget.extendedSpaceBetween) return;
    // extend the gird
    spaceBetween = widget.extendedSpaceBetween;
    setState(() {});
  }
  ...
  _onEndScroll(ScrollMetrics metrics) async {
    ...
    final double finalEnd = widget.scrollDirection == Axis.horizontal
        ? widget.crossAxisSpacing
        : widget.mainAxisSpacing;

    // avoid fast user touches cliches
    if (finalEnd == spaceBetween) return;

    for (int i = widget.extendedSpaceBetween.toInt(); i >= finalEnd; i--) {
      spaceBetween = i.toDouble();
      await Future.delayed(
          Duration(milliseconds: _duration.inMilliseconds), () {});
      setState(() {});
    }
  }
  ...
  Widget build(BuildContext context) {
    return NotificationListener<ScrollNotification>(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (cm, index) {
          return widget.children[index];
        },
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
          crossAxisCount: widget.crossAxisCount,
          crossAxisSpacing: widget.scrollDirection == Axis.horizontal
              ? spaceBetween
              : widget.crossAxisSpacing,
          mainAxisExtent: widget.mainAxisExtent,
          childAspectRatio: widget.childAspectRatio,
          mainAxisSpacing: widget.scrollDirection == Axis.vertical
              ? spaceBetween
              : widget.mainAxisSpacing,
        ),
      ),
